...
import { SlidersHorizontal, Wand2, BrainCircuit, History, Save, FolderDown, Share2, Users } from 'lucide-react';
...
const MOCK_USERS = ['user_alpha', 'user_beta', 'user_gamma', 'global'];
async function fetchPresets(userId: string): Promise<Preset[]> {
  const res = await fetch(`/api/presets?userId=${userId}`);
  if (!res.ok) throw new Error('Failed to fetch presets');
  const data: ApiResponse<Preset[]> = await res.json();
  if (!data.success) throw new Error(data.error || 'API error');
  return data.data || [];
}
async function savePreset(params: { preset: Omit<Preset, 'id'>, userId: string }): Promise<Preset[]> {
  const { preset, userId } = params;
  const res = await fetch(`/api/presets?userId=${userId}`, {
    method: 'POST',
...
export function HomePage() {
  ...
  const [userId, setUserId] = useState(() => localStorage.getItem('mockUserId') || 'global');
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (weightsParam) {
      try {
        const decodedWeights = JSON.parse(atob(weightsParam));
        setWeights(decodedWeights);
        toast.success('Shared weights loaded!');
      } catch (e) {
        console.error("Failed to parse weights from URL", e);
        toast.error('Could not load shared weights.');
      }
    }
  ...
  const handleSavePreset = () => {
    ...
    savePresetMutation.mutate({ preset: { name: presetName, weights }, userId });
  };
  ...
  const handleShare = () => {
    const encodedWeights = btoa(JSON.stringify(weights));
    const url = `${window.location.origin}${window.location.pathname}?weights=${encodedWeights}`;
            ...
            <main className="space-y-10">
                        ...
                        <SheetContent className="w-[400px] sm:w-[540px] overflow-y-auto">
                          ...
                          <div className="py-4 space-y-6">
                            <div className="space-y-4 p-4 border rounded-lg">
                                <h3 className="font-semibold flex items-center gap-2"><Users className="size-4" /> User Profile</h3>
                            ...
                            <Button onClick={handleShare} variant="outline" className="w-full">
                                <Share2 className="mr-2 size-4" /> Share Current Weights
                            </Button>
                          </div>
                        </SheetContent>
                      </Sheet>
              ...
              <section className="space-y-4">
                ...
                {isLoading ? (
                  ...
                  >
                    <ResultsCard title="Set A" description="Conservative" numbers={generatedSets.setA} scores={scores} color={NEON_COLORS.pink} onRegenerate={handleGenerate} onNumberClick={openDetailModal} />
                    <ResultsCard title="Set B" description="Spread" numbers={generatedSets.setB} scores={scores} color={NEON_COLORS.cyan} onRegenerate={handleGenerate} onNumberClick={openDetailModal} />
                    <ResultsCard title="Set C" description="Exploratory" numbers={generatedSets.setC} scores={scores} color={NEON_COLORS.orange} onRegenerate={handleGenerate} onNumberClick={openDetailModal} />
                  </motion.div>
                ) : (
